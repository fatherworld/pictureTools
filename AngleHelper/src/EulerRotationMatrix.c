...
void FromQuarternion(const float* quart,float* eular){

#undef FUNC_CODE
#define FUNC_CODE 0x0A

    const float x = quart[0];
    const float y = quart[1];
    const float z = quart[2];
    const float w = quart[3];

    eular[2] = (float)atan2(2 * (w * z + y * x), 1 - 2 * (x * x + z * z));
    eular[0] = (float)asin (2 * (w * x - z * y));
    eular[1] = (float)atan2(2 * (w * y + z * x), 1 - 2 * (x * x + y * y));
}
