...
#define FUNC_CODE 0x03
    ...
    const float x = eular[2] / 2.0f;
    const float y = eular[0] / 2.0f;
    const float z = eular[1] / 2.0f;

    quart[0] = (float)(cos(x) * sin(y) * cos(z) + sin(x) * cos(y) * sin(z));
    quart[1] = (float)(cos(x) * cos(y) * sin(z) - sin(x) * sin(y) * cos(z));
    quart[2] = (float)(sin(x) * cos(y) * cos(z) - cos(x) * sin(y) * sin(z));
    quart[3] = (float)(cos(x) * cos(y) * cos(z) + sin(x) * sin(y) * sin(z));
}
