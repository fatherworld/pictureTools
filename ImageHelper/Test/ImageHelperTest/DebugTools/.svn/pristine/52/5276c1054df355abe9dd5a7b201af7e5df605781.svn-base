...
static int dump_matrix_double(char* filename, void* buf, int col, int row) {
#undef FUNC_CODE
#define FUNC_CODE 0x0B

    double* matrix = (double*)buf;
...
#define FUNC_CODE 0x0C
    ...
    switch (DATATYPE) {
        case DUMP_FLOAT:
            ...
            break;
        case DUMP_DOUBLE:
            err = dump_matrix_double(filename, buf, col, row);
            break;
        default:
            break;
    }
...
#define FUNC_CODE 0x0D
    ...
    if(NULL == *buf) {
        *buf = malloc(tRow * col * sizeof(float));
    }
    matrix = (float*)*buf;
...
static int load_matrix_double(char* filename, void** buf, int col, int row) {
#undef FUNC_CODE
#define FUNC_CODE 0x0E
    ...
    if(NULL == *buf) {
        *buf = malloc(tRow * col * sizeof(double));
    }
    matrix = (double*)*buf;
    ...
    for(r = 0; r < tRow; r++) {
        for(c = 0; c < col; c++) {
            sscanf_fl(ptr, "%lf", &(matrix[r * col + c]));
            ptr = strchr(ptr, ',') + 1;
        }
    }
...
#define FUNC_CODE 0x0F
    ...
    switch (DATATYPE) {
        case DUMP_FLOAT:
            ...
            break;
        case DUMP_DOUBLE:
            err = load_matrix_double(filename, buf, col, row);
            break;
        default:
            break;
    }
