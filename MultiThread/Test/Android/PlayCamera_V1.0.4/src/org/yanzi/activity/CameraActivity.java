...
//		stopGoogleFaceDetect();
		...
		CameraInterface.getInstance().doStartPreview(surfaceView.getSurfaceHolder(), previewRate, this);
		mMainHandler.sendEmptyMessageDelayed(EventUtil.CAMERA_HAS_STARTED_PREVIEW, 1500);
		// startGoogleFaceDetect();

		int format = CameraInterface.getInstance().getCameraFormat();
		int height = CameraInterface.getInstance().getCameraHight();
		int width = CameraInterface.getInstance().getCameraWidth();
		int orcation = CameraInterface.getInstance().getCameraOraction();
		Point screenMetrics = DisplayUtil.getScreenMetrics(this);
		//ARJni.FRSetDataInfo(width, height, format, orcation, newId);
		
		ARJni.PalmSetDataInfo(SavePalm.palmInfo, width, height, 11, orcation, newId);
		Log.e("zhangxin", "布screenMetrics文件orcation="+ orcation +" width="+width + " height=" + height + "-id= "+ newId+"screenMetrics.x"+screenMetrics.x+"screenMetrics.y"+screenMetrics.y);
